...
namespace AuroraWorld.Gameplay.World
{
    public class Geography
    {
        ...
        private static float _vegetationSeed;
        ...
        public Geography(string seed)
        {
            ...
            _vegetationSeed = random.Next(-6 * (int)details, 7 * (int)details) / (details / 100);
        }
        ...
        public GeographyInfo InstanceInfo(Vector3Int cubePosition, out bool isContinent, out bool isRives)
        {
            ...
            var continentElevation = FractionalBrownianMotion(axial.x, axial.y, _continentSeed, 0.004f, 8);
            //continentElevation = Mathf.Pow(continentElevation, 3);
            isContinent = continentElevation >= 0.1f;

            var rivesElevation = FractionalBrownianMotion(axial.x, axial.y, _continentSeed, 0.01f, 10);
            ...
            var vegetation = FractionalBrownianMotion(axial.x, axial.y, _vegetationSeed, 0.07f, 3); 
            
            var elevation = FractionalBrownianMotion(axial.x, axial.y, _elevationSeed, 0.01f, 3) + Mathf.Clamp(continentElevation, 0, 0.1f);
            var temperature = FractionalBrownianMotion(axial.x, axial.y, _temperatureSeed, 0.01f, 3);
            var humidity = FractionalBrownianMotion(axial.x, axial.y, _humiditySeed, 0.01f, 3);
            ...
            return new GeographyInfo(elevationInt, temperature, vegetation, humidity, terrainBiome);
        }
        ...
        private static float FractionalBrownianMotion(int x, int y, float seed, float scale, int octaves = 5)
        {
            var total = 0f;
            var frequency = 1f;
            var amplitude = 0f;
