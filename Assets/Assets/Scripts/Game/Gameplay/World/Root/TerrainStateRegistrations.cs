...
using UnityEngine;
using Proxy_HexagonProxy = AuroraWorld.Gameplay.World.Terrain.Proxy.HexagonProxy;

namespace AuroraWorld.Gameplay.World.Root
{
    public static class TerrainStateRegistrations
    {
        public static void RegisterOrLoad(TerrainStateProxy terrain, List<Proxy_HexagonProxy> hexagons)
        {
            if (hexagons.Count == 0)
            {
